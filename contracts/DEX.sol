...
contract DEX {
    ...
    function ethToToken() public payable returns (uint256 tokenOutput) {
        require(msg.value > 0, "revert: eth must not be 0");
        uint256 xReserve = address(this).balance.sub(msg.value);
        uint256 yReserve = tokenReserve();

        tokenOutput = price(msg.value, xReserve, yReserve);

        require(
            token.transfer(msg.sender, tokenOutput),
            "revert: tokens failed to transfer"
        );
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        require(tokenInput > 0, "revert: cannot be zero tokens");
        uint256 xReserve = tokenReserve();
        ...
        require(sent, "revert: eth trasnfer failed");
        ...
        returns (uint256 eth_amount, uint256 token_amount)
    {
        ...
        uint256 tokenAmount = amount.mul(tokenReserves) / totalLiquidity;
